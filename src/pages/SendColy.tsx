...
const SearchableSelect = ({ value, onChange, options, placeholder, disabled = false }: any) => {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const containerRef = useRef<HTMLDivElement>(null);
  const filtered = useMemo(() => (!search ? options.slice(0, 50) : options.filter((o: string) => o.toLowerCase().includes(search.toLowerCase())).slice(0, 50)), [options, search]);
  ...
  return (
    <div ref={containerRef} className="relative">
      <button type="button" disabled={disabled} onClick={() => { if (!disabled) { setOpen(!open); setSearch(""); } }} className="flex h-10 w-full items-center justify-between rounded-xl border border-input bg-background px-3 py-2 text-sm disabled:opacity-50">
        <span className={value ? "text-foreground" : "text-muted-foreground"}>{value || placeholder}</span><ChevronDown className="h-4 w-4 opacity-50" />
      </button>
      {open && (<div className="absolute left-0 right-0 top-full mt-1 z-50 bg-card border border-border rounded-md shadow-lg"><div className="p-2"><Input autoFocus placeholder="Rechercher..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 text-sm" /></div><div className="max-h-48 overflow-y-auto">{filtered.length === 0 ? <div className="px-3 py-2 text-sm text-muted-foreground">Aucun résultat</div> : filtered.map((item: string) => (<button key={item} type="button" onClick={() => { onChange(item); setOpen(false); setSearch(""); }} className={`w-full text-left px-3 py-2 text-sm hover:bg-muted/80 transition-colors ${item === value ? "bg-primary/10 font-medium" : ""}`}>{item}</button>))}</div></div>)}
    </div>
  );
};
