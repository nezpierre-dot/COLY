...
const ResetPassword = () => {
  ...
  if (success) {
    return (
      <AuthLayout title="C'est fait !" subtitle="Votre mot de passe a été mis à jour.">
    ...
    <AuthLayout title="Nouveau mot de passe" subtitle="Choisissez un nouveau mot de passe pour votre compte.">
      <div className="flex flex-col gap-4 flex-1">
        ...
        <div className="relative">
          <input className={inputClass} placeholder="Nouveau mot de passe" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} />
          <button type="button" className="absolute right-0 top-3 text-coly-purple" onClick={() => setShowPassword(!showPassword)}>
            {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
          </button>
        </div>
        <input className={inputClass} placeholder="Confirmer le mot de passe" type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
        <div className="flex items-center justify-end pt-4">
          <button onClick={handleReset} disabled={loading} className="flex items-center gap-2 px-8 py-3 rounded-full bg-coly-orange text-white text-lg font-medium hover:opacity-90 transition-opacity shadow-lg disabled:opacity-50">
            {loading ? "..." : "Valider"} <ArrowRight size={20} />
          </button>
        </div>
      </div>
    </AuthLayout>
