...
const contactOptions = [
  {
    ...
    desc: "Réponse en moins de 5 min (9h-22h)",
    action: () => toast.info("Le chat en direct sera bientôt disponible"),
    color: "text-primary",
  ...
  },
  {
    icon: FileText,
    label: "FAQ",
    desc: "Trouvez une réponse instantanément",
    action: "faq",
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border px-5 pt-12 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[22px] font-bold text-foreground">Aide</h1>
            <p className="text-sm text-muted-foreground">Centre d'aide & support</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 py-5 space-y-7">
        {/* Contact support */}
        <section>
          <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Contacter le support</h2>
            ...
            {contactOptions.map((opt, i) => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() => {
                    hapticLight();
                    if (opt.action === "faq") navigate("/faq");
                    else if (typeof opt.action === "function") opt.action();
                  }}
                  className="w-full flex items-center gap-3.5 bg-card border border-border rounded-2xl px-4 py-3.5 text-left hover:bg-muted/30 transition-colors"
                >
        ...
        {/* Guides */}
        <section>
          <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Guides pratiques</h2>
        ...
        {/* Useful links */}
        <section>
          <h2 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">Liens utiles</h2>
          <div className="space-y-2">
            {[
              { label: "Conditions d'utilisation", route: "/terms" },
              { label: "Confidentialité & mentions légales", route: "/confidentialite" },
              { label: "Vérifier mon identité (KYC)", route: "/kyc" },
            ].map((link, i) => (
              <motion.button
                key={i}
                whileTap={{ scale: 0.98 }}
                onClick={() => { hapticLight(); navigate(link.route); }}
                className="w-full flex items-center justify-between bg-card border border-border rounded-2xl px-4 py-3.5 text-left hover:bg-muted/30 transition-colors"
              >
