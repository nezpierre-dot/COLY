CREATE OR REPLACE FUNCTION public.get_matching_voyageurs(_destination_country text, _destination_city text DEFAULT NULL::text, _departure_date date DEFAULT NULL::date, _max_weight_kg numeric DEFAULT NULL::numeric, _limit integer DEFAULT 5)
 RETURNS TABLE(voyageur_id uuid, full_name text, avatar_url text, departure_city text, departure_country text, arrival_city text, arrival_country text, departure_date date, transport_method text, max_weight_kg numeric, max_items integer, average_score numeric, total_ratings bigint, voyage_id uuid)
 LANGUAGE plpgsql
...
BEGIN
  ...
  WHERE v.status = 'active'
    ...
    -- Enforce cutoff: voyage must be at least cutoff_hours away
