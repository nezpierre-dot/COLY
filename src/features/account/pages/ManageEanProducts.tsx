...
const ManageEanProducts = () => {
  ...
  const handleAdd = async () => {
    if (!newEan.trim() || !newName.trim()) { toast.error(t("ean.errCodeName")); return; }
    if (!/^\d{8,14}$/.test(newEan.trim())) { toast.error(t("ean.errCodeFormat")); return; }
    setSaving(true);
    ...
    setSaving(false);
    if (error) { toast.error(t("ean.errAdd")); } else { toast.success(t("ean.added")); setDialogOpen(false); resetForm(); loadProducts(); }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(null);
    if (error) { toast.error(t("ean.errDelete")); } else { toast.success(t("ean.deleted")); setProducts((prev) => prev.filter((p) => p.id !== id)); }
  };

  const handleExportCsv = () => {
    if (products.length === 0) { toast.error(t("ean.errExport")); return; }
    ...
    toast.success(t("ean.exported", { count: products.length }));
  };
  ...
  const handleCsvImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (lines.length < 2) { toast.error(t("ean.errCsvEmpty")); setCsvImporting(false); return; }
      ...
      if (eanIdx === -1 || nameIdx === -1) { toast.error(t("ean.errCsvCols")); setCsvImporting(false); return; }
      ...
      if (error) { toast.error(t("ean.errCsvImport")); } else { toast.success(t("ean.imported", { count: data?.imported || 0 })); loadProducts(); }
    } catch { toast.error(t("ean.errCsvRead")); } finally { setCsvImporting(false); if (fileInputRef.current) fileInputRef.current.value = ""; }
  };
    ...
    <div className="page-shell">
      <div className="px-6 pt-12">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-[26px] font-bold text-foreground leading-tight">{t("ean.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground mb-6 pl-10">{t("ean.subtitle")}</p>

        {showScanner && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-foreground">{t("ean.scanTitle")}</p>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          <Button variant="outline" size="sm" onClick={() => setShowScanner(!showScanner)} className="gap-1.5">
            <ScanBarcode size={16} />{t("ean.scan")}
          </Button>
          <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} disabled={csvImporting} className="gap-1.5">
            {csvImporting ? <Loader2 size={16} className="animate-spin" /> : <FileSpreadsheet size={16} />}{t("ean.importCsv")}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportCsv} className="gap-1.5">
            <Download size={16} />{t("ean.exportCsv")}
          </Button>
        ...
        <div className="flex gap-2 mb-6">
          <div className="relative flex-1">
            ...
            <Input placeholder={t("ean.searchPlaceholder")} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" />
          </div>
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2"><ScanBarcode size={20} />{t("ean.addTitle")}</DialogTitle>
              </DialogHeader>
              <div className="space-y-3 mt-2">
                <Input placeholder={t("ean.codePh")} value={newEan} onChange={(e) => setNewEan(e.target.value.replace(/\D/g, ""))} maxLength={14} />
                <Input placeholder={t("ean.namePh")} value={newName} onChange={(e) => setNewName(e.target.value)} />
                <Input placeholder={t("ean.brandPh")} value={newBrand} onChange={(e) => setNewBrand(e.target.value)} />
                <div className="grid grid-cols-2 gap-3">
                  <Input placeholder={t("ean.weightPh")} value={newWeight} onChange={(e) => setNewWeight(e.target.value)} />
                  <Input placeholder={t("ean.categoryPh")} value={newCategory} onChange={(e) => setNewCategory(e.target.value)} />
                </div>
                <Input placeholder={t("ean.imagePh")} value={newImageUrl} onChange={(e) => setNewImageUrl(e.target.value)} />
                <Button onClick={handleAdd} disabled={saving} className="w-full">
                  {saving ? <Loader2 size={16} className="animate-spin mr-2" /> : <Plus size={16} className="mr-2" />}{t("ean.addBtn")}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
        ...
        <div className="mb-4 p-3 rounded-xl bg-muted/50 border border-border">
          <p className="text-xs text-muted-foreground">
            <strong>{t("ean.csvFormat")}</strong> {t("ean.csvFormatDesc")}
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground text-sm">{searchQuery ? t("ean.noResults") : t("ean.noProducts")}</p>
          </div>
        ) : (
            ...
            {products.map((p) => (
              <div key={p.id} className="bg-card rounded-xl border border-border p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-foreground text-sm font-medium truncate">{p.product_name || t("ean.noName")}</p>
